...
const StudentQuizView = () => {
  ...
  if (!activeAssignment) return <p>Loading assignment...</p>;
  ...
  const progressPercentage = ((currentQuestionIndex + 1) / activeAssignment.questions.length) * 100;
    ...
    <div className="w-full max-w-3xl bg-neutral-10 p-8 rounded-2xl shadow-2xl border border-neutral-30">
        <div className="mb-8">
            <div className="flex justify-between items-center mb-2">
                <p className="text-sm font-bold text-brand-primary">STUDENT MODE</p>
                <p className="text-sm font-semibold text-neutral-70">Question {currentQuestionIndex + 1} of {activeAssignment.questions.length}</p>
            </div>
            <div className="w-full bg-neutral-20 rounded-full h-2.5">
                <div className="bg-denotative-success h-2.5 rounded-full" style={{ width: `${progressPercentage}%` }}></div>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <button
                ...
                ${studentAnswers[currentQuestion.id] === index 
                    ...
                    : 'bg-neutral-10 border-neutral-30 hover:border-neutral-60'
                }`}
            >
                {option}
            </button>
